...
most.yards.score <- husky.games.2015[husky.games.2015$combined.yards == max.yards, 'uw_score']
...
MostYardsScore <- function(games) {
    games.scores <- games$uw_score
    games$gcombined.yards <- games$rushing_yards + games$passing_yards
    gmax.yards <- max(games$gcombined.yards)
    gmost.yards.info <- games[games$gcombined.yards == gmax.yards,]
    return (paste("The Huskies scored a total of", gmost.yards.info$gcombined.yards, 
                  "yards in their game against", gmost.yards.info$opponent, "with a score of", 
                  gmost.yards.info$uw_score, "to", gmost.yards.info$opponent_score, "on", 
                  gmost.yards.info$date, sep=" "))
}
